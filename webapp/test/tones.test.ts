...
describe('describing shore power for the marina dashboard', () => {
  it('shows no data when the mains LED has never been reported', () => {
    ...
    const result = describeShorePower(null, null)
    expect(result.value).toBe(NO_DATA)
    expect(result.tone).toBe('normal')
  })

  it('shows OFF, with a warn tone, when the mains LED reports off', () => {
    const result = describeShorePower(0, 0)
    expect(result.value).toBe('OFF')
    expect(result.tone).toBe('warn')
  })

  it('shows the wattage once the mains LED confirms shore power is on', () => {
    const result = describeShorePower(230, 1)
  ...
  it('trusts the mains LED over a near-zero float-charge reading', () => {
    // The exact bug found live aboard: 96% state of charge, Multiplus in
    // float, AC-in power reading a couple of watts — indistinguishable from
    // unplugged by wattage alone — while the panel's own mains LED, and the
    // mains, stayed on the whole time.
    const result = describeShorePower(0, 1)
    expect(result.value).not.toBe('OFF')
    expect(result.unit).toBe('W')
    expect(result.tone).toBe('normal')
  })
