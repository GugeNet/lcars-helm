 ...
 * `/signalk` — REST and the delta WebSocket — and `/plugins` — the lcars-helm
 * plugin's own status/situation routes — through to a Signal K server on this
 * machine, so the code is identical in both settings.
 */
